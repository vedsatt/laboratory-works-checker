...
// Запускает сервер без поддержки TLS
// Функция используется для тестирования при разработке
// Перед выводом сервера в продакшн следует получить TLS сертификат и
// заменить эту функцию на закомиченную функцию сверху
func (s *Server) Start() {
	submit := http.HandlerFunc(SubmitLabHandler)
